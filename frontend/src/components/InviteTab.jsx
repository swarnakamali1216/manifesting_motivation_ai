    ...
    <div style={{ padding:"4px 0" }}>
      ...
      <div style={{ position:"relative", overflow:"hidden", borderRadius:20, marginBottom:20, background:"linear-gradient(135deg,#7c5cfc 0%,#a855f7 50%,#ec4899 100%)", padding:"28px 24px 24px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:12, position:"relative", zIndex:1 }}>
          <div style={{ width:56, height:56, borderRadius:16, background:"rgba(255,255,255,0.15)", backdropFilter:"blur(8px)", display:"flex", alignItems:"center", justifyContent:"center", border:"1.5px solid rgba(255,255,255,0.25)", boxShadow:"0 4px 16px rgba(0,0,0,0.15)" }}>
            <ButterflyLogo size={34}/>
          </div>
          <div>
            <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:900, fontSize:20, color:"white", lineHeight:1.15 }}>Invite Friends</div>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", position:"relative", zIndex:1 }}>
          {[["🦋","You get +50 XP"],["🎁","Friend gets +25 XP"],["🏆","Climb ranks together"]].map(function(r,i){
            return (
              <div key={i} style={{ padding:"5px 12px", borderRadius:20, background:"rgba(255,255,255,0.15)", backdropFilter:"blur(4px)", border:"1px solid rgba(255,255,255,0.2)", fontSize:11, color:"white", fontWeight:700, display:"flex", alignItems:"center", gap:5 }}>
      ...
      <div style={{ padding:"12px 16px", borderRadius:14, background:"rgba(124,92,252,0.06)", border:"1px solid rgba(124,92,252,0.18)", marginBottom:18, display:"flex", alignItems:"center", gap:12 }}>
        <div style={{ width:38, height:38, borderRadius:"50%", flexShrink:0, background:"linear-gradient(135deg,#7c5cfc,#fc5cf0)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, fontWeight:900, color:"white" }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:20 }}>
        ...
        <button onClick={copyLink} className="inv-btn"
          style={{ padding:"11px 18px", borderRadius:12, cursor:"pointer", background: copied ? "rgba(74,222,128,0.15)" : "linear-gradient(135deg,#7c5cfc,#9c7cfc)", border: copied ? "1px solid rgba(74,222,128,0.4)" : "1px solid transparent", color: copied ? "#4ade80" : "white", fontSize:13, fontWeight:800, fontFamily:"'Syne',sans-serif", transition:"all 0.2s", whiteSpace:"nowrap" }}>
          {copied ? "Copied ✓" : "Copy 🔗"}
        </button>
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:10 }}>
        <input
          ...
          disabled={sending}
          className="inv-input"
          style={{ flex:1, padding:"11px 14px", borderRadius:12, border:"1.5px solid var(--border)", background:"var(--card)", color:"var(--text)", fontSize:13, fontFamily:"'DM Sans',sans-serif", transition:"border-color 0.2s, box-shadow 0.2s" }}/>
        <button onClick={sendEmail} disabled={sending} className="inv-btn"
          style={{ padding:"11px 22px", borderRadius:12, border:"none", cursor:sending?"not-allowed":"pointer", background: sending ? "rgba(124,92,252,0.35)" : "linear-gradient(135deg,#7c5cfc,#9c7cfc)", color:"white", fontSize:13, fontWeight:800, fontFamily:"'Syne',sans-serif", transition:"all 0.2s", whiteSpace:"nowrap" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:18 }}>
        <button className="social-btn"
          ...
          style={{ padding:"13px", borderRadius:12, cursor:"pointer", fontWeight:700, fontSize:13, background:"rgba(37,162,68,0.08)", border:"1.5px solid rgba(37,162,68,0.22)", color:"#25a244", transition:"all 0.18s" }}>
          💬 WhatsApp
        </button>
        <button className="social-btn"
          onClick={function(){ window.open("https://www.linkedin.com/sharing/share-offsite/?url="+encodeURIComponent(inviteLink),"_blank"); }}
          style={{ padding:"13px", borderRadius:12, cursor:"pointer", fontWeight:700, fontSize:13, background:"rgba(10,102,194,0.08)", border:"1.5px solid rgba(10,102,194,0.22)", color:"#0a66c2", transition:"all 0.18s" }}>
          💼 LinkedIn
        </button>
      </div>
