...
// ── Mini flying butterfly ─────────────────────────────────────────────────
function FloatingButterfly({ x, y, size, delay }) {
  var s = size || 20;
  return (
    <div style={{position:"absolute",left:x+"%",top:y+"%",opacity:0.5,pointerEvents:"none",animation:`obfly ${5+delay}s ease-in-out ${delay}s infinite`}}>
      <style>{`
      ...
      `}</style>
      <svg width={s} height={s} viewBox="0 0 40 40" fill="none" style={{animation:`obwing .7s ease-in-out ${delay}s infinite`}}>
...
// Main butterfly SVG used in branding
function BigButterfly({ size }) {
  var s = size || 64;
  return (
    <svg width={s} height={s} viewBox="0 0 40 40" fill="none" style={{filter:"drop-shadow(0 0 14px rgba(124,92,252,0.4))"}}>
  ...
  return (
    <div style={{display:"flex",gap:5,marginBottom:28}}>
      {Array.from({length:total}).map(function(_,i){
        return <div key={i} style={{flex:1,height:3,borderRadius:2,background:i<step?"var(--accent,#7c5cfc)":i===step?"rgba(124,92,252,0.35)":"rgba(255,255,255,0.1)",transition:"all .4s"}}/>;
      })}
    </div>
  );
}
...
function Onboarding({ user, onComplete }) {
  ...
  var btnStyle  = {padding:"13px 22px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#7c5cfc,#fc5cf0)",color:"#fff",fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:14,cursor:"pointer",boxShadow:"0 6px 22px rgba(124,92,252,.35)",transition:"all .18s",WebkitTapHighlightColor:"transparent"};
  var backStyle = {padding:"13px 16px",borderRadius:12,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",color:"rgba(238,238,255,0.6)",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:14,cursor:"pointer",WebkitTapHighlightColor:"transparent"};

  return (
    <div style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(5,5,15,0.94)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(10px)"}}>
      <style>{`
        ...
        .ob-input:focus{border-color:var(--accent,#7c5cfc)!important;outline:none;box-shadow:0 0 0 3px rgba(124,92,252,.15)!important;}
      `}</style>

      {/* Floating background butterflies */}
      {[{x:5,y:10,s:16,d:0},{x:85,y:5,s:20,d:1.5},{x:10,y:80,s:14,d:2.5},{x:88,y:75,s:18,d:1},{x:50,y:3,s:13,d:3}].map(function(b,i){
        return <FloatingButterfly key={i} x={b.x} y={b.y} size={b.s} delay={b.d}/>;
      })}

      <div style={{background:"linear-gradient(160deg,#0f0f22 0%,#0a0a1a 100%)",border:"1px solid rgba(124,92,252,0.25)",borderRadius:22,padding:"34px 30px",width:"100%",maxWidth:460,boxShadow:"0 40px 80px rgba(0,0,0,.6)",animation:"fadeIn .35s ease",position:"relative",zIndex:1}}>
        ...
        {step===0 && (
          <div style={{textAlign:"center",animation:"fadeIn .3s ease"}}>
            ...
            <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:24,color:"#eeeeff",marginBottom:11,letterSpacing:"-.4px"}}>Welcome to Manifesting Motivation</h2>
            <p style={{fontSize:14,color:"rgba(238,238,255,0.5)",lineHeight:1.75,marginBottom:28}}>
              You're about to have an AI coach that actually listens. Set up takes 2 minutes. Let's build your journey together.
            </p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:28}}>
              {[{icon:"🎯",text:"AI goal roadmaps"},{icon:"🔥",text:"50 badges to earn"},{icon:"📔",text:"Encrypted journal"},{icon:"⚡",text:"XP & level system"}].map(function(f){
                return (
                  <div key={f.text} style={{padding:"11px 13px",borderRadius:11,background:"rgba(124,92,252,0.08)",border:"1px solid rgba(124,92,252,0.15)",display:"flex",alignItems:"center",gap:8}}>
        ...
        {step===1 && (
          <div style={{animation:"fadeIn .3s ease"}}>
            <div style={{fontSize:10,fontWeight:800,color:"rgba(124,92,252,0.8)",letterSpacing:".14em",marginBottom:9,fontFamily:"'Syne',sans-serif"}}>STEP 1 OF 4</div>
            <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:21,color:"#eeeeff",marginBottom:7}}>What should I call you?</h2>
            <p style={{fontSize:13,color:"rgba(238,238,255,0.4)",marginBottom:20,lineHeight:1.6}}>The AI uses your name in every session.</p>
            <input className="ob-input" placeholder="Your first name..." value={name}
              onChange={function(e){setName(e.target.value);}}
              onKeyDown={function(e){if(e.key==="Enter"&&name.trim())next();}}
              autoFocus
              style={{width:"100%",padding:"13px 15px",borderRadius:12,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.12)",color:"#eeeeff",fontSize:15,fontFamily:"'DM Sans',sans-serif",marginBottom:20,boxSizing:"border-box"}}/>
            <div style={{display:"flex",gap:9}}>
              <button onClick={back} style={backStyle}>← Back</button>
              <button onClick={next} disabled={!name.trim()} style={{...btnStyle,flex:1,opacity:name.trim()?1:.4}}>
        ...
        {step===2 && (
          <div style={{animation:"fadeIn .3s ease"}}>
            <div style={{fontSize:10,fontWeight:800,color:"rgba(124,92,252,0.8)",letterSpacing:".14em",marginBottom:9,fontFamily:"'Syne',sans-serif"}}>STEP 2 OF 4</div>
            <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:21,color:"#eeeeff",marginBottom:7}}>What's your first goal, {firstName}?</h2>
            <p style={{fontSize:13,color:"rgba(238,238,255,0.4)",marginBottom:17,lineHeight:1.6}}>Don't overthink — you can add more later.</p>
            <input className="ob-input" placeholder="e.g. Get fit, learn Python, crack UPSC..." value={goalTitle}
              onChange={function(e){setGoalTitle(e.target.value);}}
              onKeyDown={function(e){if(e.key==="Enter")next();}}
              autoFocus
              style={{width:"100%",padding:"13px 15px",borderRadius:12,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.12)",color:"#eeeeff",fontSize:15,fontFamily:"'DM Sans',sans-serif",marginBottom:13,boxSizing:"border-box"}}/>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:20}}>
              {GOAL_CATS.map(function(c){
                var sel = goalCat===c.id;
                return <button key={c.id} onClick={function(){setGoalCat(c.id);}} style={{padding:"9px 6px",borderRadius:10,cursor:"pointer",textAlign:"center",border:sel?"1px solid rgba(124,92,252,0.6)":"1px solid rgba(255,255,255,0.08)",background:sel?"rgba(124,92,252,0.15)":"rgba(255,255,255,0.03)",transition:"all .15s",WebkitTapHighlightColor:"transparent"}}>
            ...
            <div style={{display:"flex",gap:9}}>
              <button onClick={back} style={backStyle}>← Back</button>
              <button onClick={next} style={{...btnStyle,flex:1}}>{goalTitle.trim()?"Save & continue →":"Skip for now →"}</button>
            </div>
          </div>
        )}
        ...
        {step===3 && (
          <div style={{animation:"fadeIn .3s ease"}}>
            <div style={{fontSize:10,fontWeight:800,color:"rgba(124,92,252,0.8)",letterSpacing:".14em",marginBottom:9,fontFamily:"'Syne',sans-serif"}}>STEP 3 OF 4</div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:20}}>
              {COACHING_STYLES.map(function(s){
                var sel = style===s.id;
                return <button key={s.id} onClick={function(){setStyle(s.id);}} style={{display:"flex",alignItems:"center",gap:13,padding:"13px 15px",borderRadius:13,cursor:"pointer",border:sel?"1px solid rgba(124,92,252,0.5)":"1px solid rgba(255,255,255,0.08)",background:sel?"rgba(124,92,252,0.1)":"rgba(255,255,255,0.02)",transition:"all .18s",textAlign:"left",WebkitTapHighlightColor:"transparent"}}>
        ...
        {step===4 && (
          <div style={{textAlign:"center",animation:"fadeIn .3s ease"}}>
            ...
            <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:22,color:"#eeeeff",marginBottom:9}}>You're all set, {firstName}!</h2>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:24}}>
              {[
                ...
                {done:!!goalTitle.trim(), text:goalTitle.trim()?"First goal: "+goalTitle.slice(0,28):"No goal yet — add one in My Goals"},
                {done:true,  text:"Coaching style: "+COACHING_STYLES.find(function(s){return s.id===style;})?.label},
                {done:true,  text:"+50 XP earned 🎉"},
              ].map(function(item){
                return <div key={item.text} style={{display:"flex",alignItems:"center",gap:9,padding:"9px 13px",borderRadius:11,background:item.done?"rgba(74,222,128,0.07)":"rgba(255,255,255,0.03)",border:"1px solid "+(item.done?"rgba(74,222,128,0.2)":"rgba(255,255,255,0.07)")}}>
            ...
            <button onClick={saveAndFinish} disabled={saving} style={{...btnStyle,width:"100%",fontSize:15,padding:15,opacity:saving?.7:1}}>
