/**
 * Home.jsx — Fixed voice consistency
 * - Uses shared voiceManager.js (same voice across all pages)
 * - Persona switch triggers notifyPersonaChanged() → voice gender updates
 * - Voice set in Settings → notifyVoiceChanged() → all pages update instantly
 */
...
var PERSONAS = [
  { id: "mentor",       emoji: "🎓", label: "Mentor",    desc: "Wise guidance & long-term thinking",  color: "#a78bfa", bg: "rgba(167,139,250,0.12)" },
  { id: "coach",        emoji: "💼", label: "Coach",     desc: "Goal-focused & accountability",        color: "#60a5fa", bg: "rgba(96,165,250,0.12)"  },
  { id: "friend",       emoji: "🤝", label: "Friend",    desc: "Warm, casual & supportive",            color: "#4ade80", bg: "rgba(74,222,128,0.12)"  },
  { id: "motivational", emoji: "🔥", label: "Hype Mode", desc: "High-energy encouragement",           color: "#fb923c", bg: "rgba(251,146,60,0.12)"  },
];
  ...
  return (
    <div ref={dropRef} style={{ position:"relative", zIndex:50 }}>
      <button
        ...
        style={{ display:"flex", alignItems:"center", gap:8, padding:"7px 12px", borderRadius:12, border:"1px solid "+active.color+"55", background:active.bg, cursor:"pointer", transition:"all 0.18s", fontFamily:"'Syne',sans-serif" }}
      >
        <span style={{ fontSize:16 }}>{active.emoji}</span>
        <div style={{ textAlign:"left" }}>
          <div style={{ fontSize:11, fontWeight:800, color:active.color, lineHeight:1 }}>{active.label}</div>
          <div style={{ fontSize:9, color:"var(--muted)", lineHeight:1.3, marginTop:1 }}>AI Persona</div>
        </div>
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ marginLeft:2, transition:"transform 0.2s", transform:open?"rotate(180deg)":"rotate(0deg)" }}>
          <path d="M2 4l4 4 4-4" stroke={active.color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </button>

      {open && (
        <div style={{ position:"absolute", top:"calc(100% + 8px)", right:0, width:240, background:"var(--card)", border:"1px solid var(--border)", borderRadius:16, boxShadow:"0 8px 32px rgba(0,0,0,0.18)", overflow:"hidden", animation:"dropIn 0.15s ease" }}>
          <div style={{ padding:"10px 14px 8px", borderBottom:"1px solid var(--border)", fontSize:10, fontWeight:800, color:"var(--muted)", letterSpacing:"0.1em", fontFamily:"'Syne',sans-serif" }}>
            ...
            return (
              <button key={p.id}
                onClick={function() {
                  ...
                  // Notify voiceManager — persona changes voice gender preference
                  notifyPersonaChanged();
                  window.dispatchEvent(new Event("personachange"));
                  setOpen(false);
                }}
                style={{ width:"100%", display:"flex", alignItems:"center", gap:12, padding:"11px 14px", border:"none", cursor:"pointer", background:isActive?p.bg:"transparent", borderLeft:isActive?"3px solid "+p.color:"3px solid transparent", transition:"all 0.15s", textAlign:"left" }}
                onMouseEnter={function(e){ if(!isActive) e.currentTarget.style.background="rgba(124,92,252,0.06)"; }}
                onMouseLeave={function(e){ if(!isActive) e.currentTarget.style.background="transparent"; }}
              >
                <div style={{ width:36, height:36, borderRadius:10, flexShrink:0, background:isActive?p.bg:"var(--bg)", border:"1px solid "+(isActive?p.color+"55":"var(--border)"), display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>
                  {p.emoji}
                </div>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:13, fontWeight:800, fontFamily:"'Syne',sans-serif", color:isActive?p.color:"var(--text)", lineHeight:1.2 }}>
                    {p.label}
                    {isActive && <span style={{ marginLeft:6, fontSize:9, padding:"1px 6px", borderRadius:6, background:p.color+"22", color:p.color, verticalAlign:"middle", fontWeight:700 }}>ACTIVE</span>}
                  </div>
                  <div style={{ fontSize:11, color:"var(--muted)", marginTop:2, lineHeight:1.3 }}>{p.desc}</div>
                </div>
                {isActive && (
          ...
          <div style={{ padding:"8px 14px", borderTop:"1px solid var(--border)", fontSize:10, color:"var(--muted)", lineHeight:1.4 }}>
...
function Home({ user, onNavigate }) {
  ...
  // Subscribe to voiceManager speaking state changes
  useEffect(function() {
    var unsub = onSpeakingChange(function(val) { setSpeaking(val); });
    return unsub;
  }, []);
  ...
  function sendMsg() {
    ...
    axios.post(API + "/motivate", {
      ...
      setMsgs(function(p) {
        return p.concat({ role:"bot", text:reply, emotion:r.data.emotion||"neutral", vader:r.data.vader_score, xp:r.data.xp_awarded||0 });
      });
      loadStats();
      speak(reply); // ← shared voiceManager: same voice as Goals & Checkin
    }).catch(function() {
      setMsgs(function(p) { return p.concat({ role:"bot", text:"Something went wrong. Is Flask running?" }); });
  ...
  function handleExport() {
    ...
    axios.get(API + "/privacy/export?user_id=" + uid, { responseType:"blob" })
      .then(function(r) {
        ...
        var a = document.createElement("a"); a.href = url; a.download = "my_data.json"; a.click();
    ...
    <div style={{ animation:"fadeIn 0.3s ease", paddingBottom:32 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:8 }}>
        ...
        <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
          ...
          <button className="topbtn" onClick={handleExport} disabled={exporting}>{exporting?"...":"Export"}</button>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, marginBottom:20 }}>
        {STATS_CFG.map(function(s) {
          return (
            <div key={s.id} className="stat-pill" onClick={function() { onNavigate(s.nav); }}
      ...
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:18, overflow:"visible", marginBottom:16, position:"relative" }}>
        <div style={{ padding:"12px 16px", borderBottom:"1px solid var(--border)", display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:13, color:"var(--text)", display:"flex", alignItems:"center", gap:6 }}>
              AI Coach
              <span style={{ fontSize:10, padding:"2px 8px", borderRadius:8, background:activePersona.bg, color:activePersona.color, border:"1px solid "+activePersona.color+"33", fontWeight:700 }}>
                {activePersona.emoji} {activePersona.label}
              </span>
              {speaking && <span style={{ fontSize:12, animation:"bounce 1s infinite" }}>🔊</span>}
            </div>
        ...
        <div style={{ padding:"7px 16px", borderBottom:"1px solid var(--border)", background:activePersona.bg, display:"flex", alignItems:"center", gap:8, fontSize:11, color:activePersona.color, transition:"background 0.3s" }}>
              ...
              <div key={i} style={{ display:"flex", flexDirection:"column", alignItems:isUser?"flex-end":"flex-start", gap:3 }}>
                ...
                {!isUser && m.emotion && m.emotion !== "neutral" && (
                  <div style={{ display:"flex", alignItems:"center", gap:4, paddingLeft:6 }}>
                    <span style={{ fontSize:9, fontWeight:700, color:emoColor, background:emoColor+"18", padding:"2px 7px", borderRadius:8, fontFamily:"'Syne',sans-serif", border:"1px solid "+emoColor+"30" }}>
                      {emoEmoji} {m.emotion.charAt(0).toUpperCase()+m.emotion.slice(1)}
                      {m.vader !== undefined && m.vader !== null ? " · "+(m.vader>0?"+":"")+Number(m.vader).toFixed(2) : ""}
                    </span>
                  </div>
                )}
              </div>
        ...
        <div style={{ padding:"10px 12px", borderTop:"1px solid var(--border)", display:"flex", gap:8 }}>
          ...
          />
          <button onClick={sendMsg} disabled={busy||!input.trim()} style={{ padding:"9px 16px", borderRadius:11, border:"none", background:input.trim()?"linear-gradient(135deg,#7c5cfc,#9c7cfc)":"var(--border)", color:"white", fontWeight:800, fontSize:13, cursor:input.trim()?"pointer":"not-allowed", fontFamily:"'Syne',sans-serif" }}>
            {busy?"...":"→"}
          </button>
        </div>
      </div>
