...
var MOODS = [
  ...
  { key:"good",     emoji:"≡ƒÿè", label:"Good",      color:"#4ade80", bg:"rgba(74,222,128,0.12)"  },
  { key:"okay",     emoji:"≡ƒÿÉ", label:"Okay",      color:"#60a5fa", bg:"rgba(96,165,250,0.12)"  },
  { key:"low",      emoji:"≡ƒÿö", label:"Low",       color:"#a78bfa", bg:"rgba(167,139,250,0.12)" },
  { key:"anxious",  emoji:"≡ƒÿ░", label:"Anxious",   color:"#fb923c", bg:"rgba(251,146,60,0.12)"  },
  { key:"grateful", emoji:"≡ƒÖÅ", label:"Grateful",  color:"#34d399", bg:"rgba(52,211,153,0.12)"  },
  { key:"angry",    emoji:"≡ƒÿñ", label:"Frustrated",color:"#f87171", bg:"rgba(248,113,113,0.12)" },
  { key:"excited",  emoji:"ΓÜí", label:"Excited",   color:"#c084fc", bg:"rgba(192,132,252,0.12)" },
];
...
function MoodStreak({ entries }) {
  ...
  var result = [];
  // IST_MS already declared above ΓÇö reuse it
  for (var i=6; i>=0; i--) {
      ...
      <div style={{ padding:"0 14px 12px" }}>
        ...
        {isLong && (
          <button onClick={function(){ setExpanded(function(v){return !v;}); }}
            ...
            {expanded ? "Show less Γåæ" : "Read more Γåô"}
          </button>
        )}
      </div>
...
function WriteModal({ user, editEntry, onClose, onSaved }) {
  ...
  function save() {
    ...
    req.then(function(){
      ...
      console.error(e);
      alert("Save failed ΓÇö check Flask terminal");
      ...
      <div style={{
        ...
        <div style={{ padding:"16px 20px 12px", display:"flex", alignItems:"center", justifyContent:"space-between", borderBottom:"1px solid var(--border)" }}>
          <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:"900", fontSize:"16px", color:"var(--text)" }}>
            {isEdit ? "Γ£Å∩╕Å Edit Entry" : "Γ£ì∩╕Å New Entry"}
          </div>
          <div style={{ display:"flex", gap:"8px", alignItems:"center" }}>
            <span style={{ fontSize:"10px", color:"var(--muted)" }}>{wordCount(content)} words</span>
            <button onClick={onClose} style={{ background:"none", border:"none", color:"var(--muted)", fontSize:"18px", cursor:"pointer", lineHeight:1 }}>├ù</button>
          </div>
        </div>
        ...
        <div style={{ overflowY:"auto", flex:1, padding:"16px 20px" }}>
          ...
          {!isEdit && prompt && (
            <div style={{ background:"rgba(124,92,252,0.06)", border:"1px solid rgba(124,92,252,0.15)", borderRadius:"12px", padding:"10px 14px", marginBottom:"14px", cursor:"pointer" }}
              ...
              <div style={{ fontSize:"9px", color:"var(--accent)", fontWeight:"800", fontFamily:"'Syne',sans-serif", marginBottom:"3px" }}>≡ƒÆí TODAY'S PROMPT ΓÇö tap to use</div>
          ...
          <textarea
            ref={textRef}
            placeholder={"Write freely... your thoughts are safe here.\n\nThis is your private space ΓÇö be honest with yourself."}
            value={content}
        ...
        <div style={{ padding:"12px 20px", borderTop:"1px solid var(--border)", display:"flex", justifyContent:"space-between", alignItems:"center", background:"var(--surface)" }}>
          <div style={{ fontSize:"10px", color:"var(--muted)", display:"flex", alignItems:"center", gap:"5px" }}>
            ≡ƒöÆ <span>Private & encrypted ΓÇö only you can see this</span>
          </div>
          <div style={{ display:"flex", gap:"8px" }}>
            ...
            <button onClick={save} disabled={saving||!content.trim()}
              ...
              {saving ? "Saving..." : isEdit ? "Update Γ£ô" : "Save Entry Γ£ô"}
            </button>
          </div>
        </div>
      </div>
...
function Journal({ user }) {
  ...
  // Use mood_score (VADER) for positive/tough ΓÇö not the mood string which is often "okay"
    ...
    <div style={{ animation:"fadeIn 0.3s ease", paddingBottom:"24px" }}>
      ...
      <div style={{ marginBottom:"16px" }}>
        ...
        <div style={{ display:"inline-flex", alignItems:"center", gap:"5px", background:"rgba(74,222,128,0.08)", border:"1px solid rgba(74,222,128,0.2)", borderRadius:"8px", padding:"4px 10px", marginTop:"4px" }}>
          <span style={{ fontSize:"10px" }}>≡ƒöÆ</span>
          <span style={{ fontSize:"10px", color:"#4ade80", fontWeight:"700", fontFamily:"'Syne',sans-serif" }}>Private & Secure ΓÇö only you can read this</span>
        </div>
      </div>
      ...
      {entries.length > 0 && (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"8px", marginBottom:"14px" }}>
          {[
            { v:posCount,       l:"Positive",   c:"#4ade80", e:"≡ƒÿè" },
            { v:toughCount,     l:"Tough",       c:"#f87171", e:"≡ƒÆ¬" },
            { v:streakDays+"d", l:"Streak",      c:"#fb923c", e:"≡ƒöÑ" },
            { v:totalWords,     l:"Words",       c:"#60a5fa", e:"Γ£ì∩╕Å" },
          ].map(function(s){
            return (
      ...
      {entries.length > 0 && (
        <div style={{ marginBottom:"12px", display:"flex", flexDirection:"column", gap:"8px" }}>
          <input
            className="j-input"
            placeholder="≡ƒöì Search entries..."
            value={search}
          ...
          <div style={{ display:"flex", gap:"6px", overflowX:"auto", paddingBottom:"2px" }}>
            ...
            <button onClick={function(){ setSortDir(function(v){return v==="desc"?"asc":"desc";}); }}
              ...
              {sortDir==="desc"?"Γåô Newest":"Γåæ Oldest"}
            </button>
          </div>
          {/* Tag filter */}
      ...
      {loading ? (
        ...
        <div style={{ textAlign:"center", padding:"48px 20px", background:"var(--card)", borderRadius:"20px", border:"1px solid var(--border)" }}>
          <div style={{ fontSize:"48px", marginBottom:"12px" }}>≡ƒôû</div>
