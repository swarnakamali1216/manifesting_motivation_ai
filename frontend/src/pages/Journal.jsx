...
function MoodStreak({ entries }) {
  ...
  entries.forEach(function(e) {
    ...
    var key = toISTDateString(e.created_at);
    if (key) days[key] = e.mood || "okay";
  });
  ...
  for (var i = 6; i >= 0; i--) {
    ...
    var key = d.toLocaleDateString("en-CA", { timeZone: "Asia/Kolkata" });
    var dayIndex = new Date(key + "T12:00:00").getDay();
    result.push({ key: key, dayLabel: DAY_LABELS[dayIndex], mood: days[key] || null });
  }
...
function Journal({ user }) {
  ...
  var filtered = entries
    ...
    .sort(function(a, b) {
      var da = parseDate(a.created_at) || new Date(0);
      var db = parseDate(b.created_at) || new Date(0);
  ...
  var streakDays = new Set(
    entries
      .map(function(e){ return toISTDateString(e.created_at); })
      .filter(function(d){ return d.length === 10; })
  ).size;

  var groups = {};
  filtered.forEach(function(e) {
    var d = parseDate(e.created_at) || new Date();
    var key = d.toLocaleDateString("en-IN", { month:"long", year:"numeric", timeZone:"Asia/Kolkata" });
